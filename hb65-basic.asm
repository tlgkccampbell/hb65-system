; hb65-basic.asm
.FILEOPT    author,     "Lee Davidson, Klaus Dormann, Michael Billington, Cole Campbell"
.FILEOPT    comment,    "Enhanced BASIC for 6502"

.IMPORTZP   EHBASIC_ZP_START, EHBASIC_ZP_MAX
.EXPORT     LAB_WARM, LAB_COLD, V_INPT, V_OUTP, VEC_IN, VEC_SV, NmiBase, IrqBase
...
; 2.22.hb65    implemented zeropage relocation
;              implemented mixed case support

.DEFINE EHBASIC_PROGRAM_NAME  "Enhanced BASIC"
.DEFINE EHBASIC_VERSION       "2.22.hb65"

; feature configuration
.IFDEF EHBASIC_ANSI_SUPPORT
      .OUT "EhBASIC: Enabled ANSI terminal code support."
.ENDIF
.IFDEF EHBASIC_MIXED_CASE
      .OUT "EhBASIC: Enabled mixed case support."
.ENDIF
.IFDEF EHBASIC_SKIP_BOOT_PROMPT
      .OUT "EhBASIC: Enabled boot prompt skip."
.ENDIF
.IFDEF EHBASIC_SKIP_MEMORY_PROMPT
      .OUT "EhBASIC: Enabled memory prompt skip."
.ENDIF
...
ZP_MAX      = EHBASIC_ZP_MAX
.ENUM
      ; the following locations are bulk initialized from StrTab at LAB_GMEM
      LAB_WARM    = ZP_START        ; BASIC warm start entry point
      Wrmjpl                        ; BASIC warm start vector jump low byte
      Wrmjph                        ; BASIC warm start vector jump high byte

      Usrjmp                        ; USR function JMP address
      Usrjpl                        ; USR function JMP vector low byte
      Usrjph                        ; USR function JMP vector high byte
      Nullct      = ZP_START + $0D  ; nulls output after each line
      TPos                          ; BASIC terminal position byte
      TWidth                        ; BASIC terminal width byte
      Iclim                         ; input column limit
      Itempl                        ; temporary integer low byte
      Itemph                        ; temporary integer high byte
      ...
      nums_3                        ; number to bin/hex string convert LSB

      Srchc                         ; search character
      Temp3       = Srchc           ; temp byte used in number routines
      Scnquo                        ; scan-between-quotes flag
      ...
      Ibptr                         ; input buffer pointer
      Dimcnt      = Ibptr           ; # of dimensions
      Tindx       = Ibptr           ; token index

      Defdim                        ; default DIM flag
      Dtypef                        ; data type flag, $FF=string, $00=numeric
      Oquote                        ; open quote flag (b7) (Flag: DATA scan; LIST quote; memory)
      Gclctd                        ; garbage collected flag
      Sufnxf                        ; subscript/FNX flag, 1xxx xxx = FN(0xxx xxx)
      Imode                         ; input mode flag, $00=INPUT, $80=READ

      Cflag                         ; comparison evaluation flag

      TabSiz                        ; TAB step size (was input flag)

      next_s                        ; next descriptor stack address

                                    ; these two bytes form a word pointer to the item
                                    ; currently on top of the descriptor stack
      last_sl                       ; last descriptor stack address low byte
      last_sh                       ; last descriptor stack address high byte (always $00)

      des_sk                        ; descriptor stack start address (temp strings)
      des_sk_end  = des_sk + $08    ; End of descriptor stack
      
      ut1_pl                        ; utility pointer 1 low byte
      ut1_ph                        ; utility pointer 1 high byte
      ut2_pl                        ; utility pointer 2 low byte
      ut2_ph                        ; utility pointer 2 high byte

      Temp_2      = ut1_pl          ; temp byte for block move    

      FACt_1      = ut2_ph+1        ; FAC temp mantissa1
      FACt_2                        ; FAC temp mantissa2
      FACt_3                        ; FAC temp mantissa3

      dims_l      = FACt_2          ; array dimension size low byte
      dims_h      = FACt_3          ; array dimension size high byte

      TempB                         ; temp page 0 byte

      Smeml                         ; start of mem low byte       (Start-of-Basic)
      Smemh                         ; start of mem high byte      (Start-of-Basic)
      Svarl                         ; start of vars low byte      (Start-of-Variables)
      Svarh                         ; start of vars high byte     (Start-of-Variables)
      Sarryl                        ; var mem end low byte        (Start-of-Arrays)
      Sarryh                        ; var mem end high byte       (Start-of-Arrays)
      Earryl                        ; array mem end low byte      (End-of-Arrays)
      Earryh                        ; array mem end high byte     (End-of-Arrays)
      Sstorl                        ; string storage low byte     (String storage (moving down))
      Sstorh                        ; string storage high byte    (String storage (moving down))
      Sutill                        ; string utility ptr low byte
      Sutilh                        ; string utility ptr high byte
      Ememl                         ; end of mem low byte         (Limit-of-memory)
      Ememh                         ; end of mem high byte        (Limit-of-memory)
      Clinel                        ; current line low byte       (Basic line number)
      Clineh                        ; current line high byte      (Basic line number)
      Blinel                        ; break line low byte         (Previous Basic line number)
      Blineh                        ; break line high byte        (Previous Basic line number)

      Cpntrl                        ; continue pointer low byte
      Cpntrh                        ; continue pointer high byte

      Dlinel                        ; current DATA line low byte
      Dlineh                        ; current DATA line high byte

      Dptrl                         ; DATA pointer low byte
      Dptrh                         ; DATA pointer high byte

      Rdptrl                        ; read pointer low byte
      Rdptrh                        ; read pointer high byte

      Varnm1                        ; current var name 1st byte
      Varnm2                        ; current var name 2nd byte

      Cvaral                        ; current var address low byte
      Cvarah                        ; current var address high byte

      Frnxtl                        ; var pointer for FOR/NEXT low byte
      Frnxth                        ; var pointer for FOR/NEXT high byte
      ...
      prstk                         ; precedence stacked flag

      comp_f                        ; compare function flag, bits 0,1 and 2 used
                                    ; bit 2 set if >
                                    ; bit 1 set if =
                                    ; bit 0 set if <

      func_l                        ; function pointer low byte
      func_h                        ; function pointer high byte

      garb_l      = func_l          ; garbage collection working pointer low byte
      garb_h      = func_h          ; garbage collection working pointer high byte

      des_2l                        ; string descriptor_2 pointer low byte
      des_2h                        ; string descriptor_2 pointer high byte

      g_step                        ; garbage collect step size

      Fnxjmp                        ; jump vector for functions
      Fnxjpl                        ; functions jump vector low byte
      Fnxjph                        ; functions jump vector high byte

      g_indx      = Fnxjpl          ; garbage collect temp index

      FAC2_r      = Fnxjph+1        ; FAC2 rounding byte

      Adatal                        ; array data pointer low byte
      Adatah                        ; array data pointer high  byte

      Nbendl      = Adatal          ; new block end pointer low byte
      Nbendh      = Adatah          ; new block end pointer high  byte

      Obendl                        ; old block end pointer low byte
      Obendh                        ; old block end pointer high  byte

      numexp                        ; string to float number exponent count
      expcnt                        ; string to float exponent count

      numbit      = numexp          ; bit count for array element calculations

      numdpf      = expcnt+1        ; string to float decimal point flag
      expneg                        ; string to float eval exponent -ve flag
      ...
      FAC1_e                        ; FAC1 exponent
      FAC1_1                        ; FAC1 mantissa1
      FAC1_2                        ; FAC1 mantissa2
      FAC1_3                        ; FAC1 mantissa3
      FAC1_s                        ; FAC1 sign (b7)
      ...
      negnum      = FAC1_s+1        ; string to float eval -ve flag
      numcon                        ; series evaluation constant count

      FAC1_o                        ; FAC1 overflow byte

      FAC2_e                        ; FAC2 exponent
      FAC2_1                        ; FAC2 mantissa1
      FAC2_2                        ; FAC2 mantissa2
      FAC2_3                        ; FAC2 mantissa3
      FAC2_s                        ; FAC2 sign (b7)

      FAC_sc                        ; FAC sign comparison, Acc#1 vs #2
      ...
      Aspth                         ; array size/pointer high byte
      ...
      LAB_IGBY                      ; get next BASIC byte subroutine

      LAB_GBYT    = LAB_IGBY+$06    ; get current BASIC byte subroutine
      Bpntrl                        ; BASIC execute (get byte) pointer low byte
      Bpntrh                        ; BASIC execute (get byte) pointer high byte
      LAB_GBYT_end= LAB_GBYT+$15    ; end of get BASIC char subroutine
      ; end bulk initialize from LAB_2CEE at LAB_2D4E

      Rbyte4                        ; extra PRNG byte
      Rbyte1                        ; most significant PRNG byte
      Rbyte2                        ; middle PRNG byte
      Rbyte3                        ; least significant PRNG byte

      NmiBase                       ; NMI handler enabled/setup/triggered flags
                                    ; bit function
                                    ; === ========
                                    ; 7   interrupt enabled
                                    ; 6   interrupt setup
                                    ; 5   interrupt happened
      ;                             ; NMI handler addr low byte
      ;                             ; NMI handler addr high byte
      IrqBase     = NmiBase+$03     ; IRQ handler enabled/setup/triggered flags
      ;                             ; IRQ handler addr low byte
      ;                             ; IRQ handler addr high byte

      Decss       = IrqBase+$03     ; number to decimal string start
      Decssp1                       ; number to decimal string start
      Decss_end   = Decss+$10       ; decimal string end

      ZP_END
.ENDENUM

.ASSERT ZP_END <= ZP_MAX, error, "not enough zeropage memory for EhBASIC"
.OUT .SPRINTF("EhBASIC: Used %d ($%02X) zeropage bytes.", ZP_END - ZP_START, ZP_END - ZP_START)
...
.IFNDEF EHBASIC_SKIP_MEMORY_PROMPT
...
LAB_2E05
      ...
      LDA   #<LAB_MMSG        ; point to sign-on message (low addr)
      LDY   #>LAB_MMSG        ; point to sign-on message (high addr)
      JSR   LAB_18C3          ; print null terminated string from memory
...
; *** add
.ASSERT * & $FF <> $FD, lderror, "LAB_15B3: potential return address -$100 (page not incremented)"
...
; *** begin patch  2.22p5.3   potential return address -$100 (page not incremented) ***
; *** add
.ASSERT * & $FF <> $FD, lderror, "LAB_1B43: potential return address -$100 (page not incremented)"
; *** end   patch  2.22p5.3   potential return address -$100 (page not incremented) ***
...
.IFDEF EHBASIC_CONDENSED_SIGNON
      .IFDEF EHBASIC_ANSI_SUPPORT
            .byte $1B, "[2J", $1B, "[;H", $1B, "[97m"
      .ENDIF
      .byte .CONCAT(EHBASIC_PROGRAM_NAME, " ", EHBASIC_VERSION)
      .IFDEF EHBASIC_ANSI_SUPPORT
            .byte $1B, "[0m"
      .ENDIF
      .byte $00
.ELSE
      .IFDEF EHBASIC_ANSI_SUPPORT
            .byte $1B, "[97m"
      .ENDIF
      .byte .CONCAT(EHBASIC_PROGRAM_NAME, " ", EHBASIC_VERSION),$0A
      .IFDEF EHBASIC_ANSI_SUPPORT
            .byte $1B, "[0m"
      .ENDIF
      .byte $00
